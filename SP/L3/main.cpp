...
// Overloading operator for custom sorting
bool operator < (const chunk& a, const chunk& b) { return a.start < b.start; }

// Search given lexem type in string
...
int main () {
    ...
    parse (str, R"(\b(?:0[bB][01]+|0[1-7]+|0[xX][0-9a-fA-F]+|\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\b)", chunks, type::NUMBER);
    parse (str, R"(\b(?:\+|\-|\*|=|\!=|\<|\<=|\>|>=|\&|\||\^|\~|\<<|\>>|\&=|\|=|\^=|\?|\:|\,|\-\>)\b)", chunks, type::OPERATOR);
    
    // Print all found lexems and their locations
    ...
    // Print file to console highlighting detected lexems
    auto it = chunks.begin ();
    int start = 0, end;
